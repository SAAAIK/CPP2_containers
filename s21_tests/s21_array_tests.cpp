...
TEST(ArrayTest, InitializerListConstructor) {
  s21::array<int, 5> myArray{1, 2, 3, 4, 5};  // Использование initializer_list
...
TEST(ArrayMoveConstructor, MoveConstructorCreatesValidObject) {
  ...
  EXPECT_EQ(moved.size(), (unsigned long)5);
  EXPECT_EQ(moved[0], 1);
  EXPECT_EQ(moved[4], 5);

  // Проверка, что original находится в валидном, но неопределенном состоянии
  EXPECT_EQ(original.size(), (unsigned long)0);
...
TEST(ArrayMoveAssignment, MoveAssignmentCreatesValidObject) {
  ...
  // Проверка, что moved содержит данные original
  EXPECT_EQ(int(moved.size()), (int)5);
  EXPECT_EQ(moved[0], 1);
  EXPECT_EQ(moved[4], 5);
...
TEST(ArrayAssignmentOperator, MoveAssignment) {
  ...
  EXPECT_EQ(moved.size(), (unsigned long)3);
  EXPECT_EQ(moved[0], 1);
  EXPECT_EQ(moved[1], 2);
  EXPECT_EQ(moved[2], 3);
...
TEST(ArrayFunctions, SizeFunction) {
  s21::array<int> arr1;  // Пустой массив
  EXPECT_EQ(arr1.size(), (unsigned long)0);

  s21::array<int> arr2{1, 2, 3};  // Непустой массив
  EXPECT_EQ(arr2.size(), (unsigned long)3);
}
...
TEST(AarrayOperatorEvenTest, OperatorEvenCopy) {
  s21::array<int> arr1{1, 2, 3};
  s21::array<int> arr2{4, 5, 6};

  arr2 = arr1;

  size_t arr1_size = arr1.size();

  ASSERT_EQ((int)arr1_size, 3);

  ASSERT_EQ((int)arr2.size(), (int)arr1_size);
  ASSERT_EQ(arr2[0], 1);
  ASSERT_EQ(arr2[1], 2);
  ASSERT_EQ(arr2[2], 3);
}

TEST(AarrayOperatorEvenTest, OperatorEvenMove) {
  s21::array<int> arr1{1, 2, 3};
  s21::array<int> arr2{4, 5, 6};
  ...
  ASSERT_EQ((int)arr1_size, 0);
  ASSERT_EQ(arr1.data(), nullptr);

  ASSERT_EQ((int)arr2.size(), 3);
  ASSERT_EQ(arr2[0], 1);
  ASSERT_EQ(arr2[1], 2);
  ASSERT_EQ(arr2[2], 3);
}