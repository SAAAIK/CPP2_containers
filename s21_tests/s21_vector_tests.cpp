...
TEST(VectorFunctions, ReserveFunction) {
  ...
  EXPECT_GE((int)vec.capacity(), 10);
  EXPECT_GE((int)vec.capacity(), (int)oldCapacity);
  ...
  vec.reserve(3);
  EXPECT_GE((int)vec.capacity(), 5);  // Вместимость не должна уменьшиться
}

// Дополнительный //  для проверки поведения reserve при размере 0
TEST(VectorFunctions, ReserveZeroSize) {
  s21::vector<int> vec;
  vec.reserve(0);

  // Проверка, что вместимость не меньше 0
  EXPECT_GE((int)vec.capacity(), 0);
}
...
TEST(VectorClearTest, ClearEmptyVector2) {
  ...
  ASSERT_EQ((int)my_vector.size(), 0);
}

// // ирование метода clear на непустом векторе
...
// // ирование повторного использования вектора после clear
TEST(VectorClearTest, ReuseAfterClear) {
  ...
  ASSERT_EQ((int)my_vector.size(), 2);
...
TEST(VectorPushBackTest, PushBackToEmptyVector) {
  ...
  ASSERT_EQ((int)my_vector.size(), 1);
  EXPECT_EQ(my_vector[0], 1);
}

// // ирование последовательного добавления элементов
TEST(VectorPushBackTest, SequentialPushBack) {
  ...
  ASSERT_EQ((int)my_vector.size(), 3);
  EXPECT_EQ(my_vector[0], 1);
...
TEST(VectorPopBackTest, PopBackFromNonEmptyVector) {
  ...
  ASSERT_EQ((int)my_vector.size(), (int)std_vector.size());
...
TEST(VectorEraseTest, EraseFromMiddle) {
  ...
  EXPECT_THROW(my_vector.erase(it_1 + 5), std::out_of_range);
}
...
TEST(VectorInsertTest, InsertAtBeginning) {
  ...
  ASSERT_EQ((int)my_vector.size(), 4);
  EXPECT_EQ(*it, 1);
  EXPECT_EQ(my_vector[0], 1);
  EXPECT_EQ(my_vector[1], 2);
  EXPECT_EQ(my_vector[2], 3);
...
TEST(VectorAssignmentTest, MoveAssignmentOperator) {
  ...
  ASSERT_EQ((int)moved.size(), 2);
  EXPECT_EQ(moved[0], 1);
  EXPECT_EQ(moved[1], 2);

  // Проверяем, что original теперь пуст
  ASSERT_EQ((int)original.size(), 0);
}
...
TEST(VectorSwapTest, SwapWithEmptyVector) {
  ...
  ASSERT_EQ((int)vec1.size(), 0);
  ASSERT_EQ((int)vec1.capacity(), 0);
...
TEST(VectorOperatorEvenTest, OperatorEvenCopy) {
  s21::vector<int> vec1{1, 2, 3};
  ...
  ASSERT_EQ((int)vec1_size, 3);
  ASSERT_EQ((int)vec1_capacity, 3);

  ASSERT_EQ((int)vec2.size(), (int)vec1_size);
  ASSERT_EQ((int)vec2.capacity(), (int)vec1_capacity);
  ASSERT_EQ(vec2[0], 1);
  ASSERT_EQ(vec2[1], 2);
  ASSERT_EQ(vec2[2], 3);
}

TEST(VectorOperatorEvenTest, OperatorEvenMove) {
  s21::vector<int> vec1{1, 2, 3};
  s21::vector<int> vec2{4, 5, 6};
  ...
  ASSERT_EQ((int)vec1_size, 0);
  ASSERT_EQ((int)vec1_capacity, 0);
  ASSERT_EQ(vec1.data(), nullptr);

  ASSERT_EQ((int)vec2.size(), 3);
  ASSERT_EQ((int)vec2.capacity(), 3);
  ASSERT_EQ(vec2[0], 1);
  ASSERT_EQ(vec2[1], 2);
  ASSERT_EQ(vec2[2], 3);
}